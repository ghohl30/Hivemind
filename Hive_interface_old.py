...
class App(Tk):
	def __init__(self):
		...
		self.can = Canvas(self, width=800, height=800, bg="#60ace6")
		self.can.pack()
		self.turn = 0

		self.hexagons = []
		self.initGrid(20, 20, 30, debug=False)
		...
		for i in self.hexagons: # re-configure selected only
			if i.selected:
				if self.turn % 2 == 0:
					...
					img = ImageTk.PhotoImage(Image.open("DoesntMatter.jpg"))
					self.can.create_image(20,20,image = img)
				else:
					self.can.itemconfigure(i.tags, fill="#000000")
			if i.isNeighbour:
